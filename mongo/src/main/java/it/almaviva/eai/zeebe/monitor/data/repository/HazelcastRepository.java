...
public class HazelcastRepository implements IHazelcastPort {
	...
	public HazelcastConfigDomain findById(String id) {
		Optional<HazelcastDocument> byId = springDataHazelcastRepository.findById(id);
		HazelcastDocument hazelcastDocument = byId.orElseGet(() -> {return null;});
		return iHazelcastMapper.map(hazelcastDocument);
	}

	@Override
	public void save(HazelcastConfigDomain hazelcastConfigDomain) {
		HazelcastDocument hazelcastDocument = new HazelcastDocument();
		hazelcastDocument.setId(hazelcastConfigDomain.getId());
		hazelcastDocument.setSequence(hazelcastConfigDomain.getSequence());
		springDataHazelcastRepository.save(hazelcastDocument);
